...
for (const f of readdirSync(dir).filter((x) => x.endsWith(".json"))) {
  ...
  if (mins < -20 || mins > 180) continue;
  cards.push(a);
}
cards.sort((a, b) => Date.parse(a.input.kickoff) - Date.parse(b.input.kickoff));
console.log("window", cards.map((c) => `${c.input.home} ${Math.round((Date.parse(c.input.kickoff) - now) / 60000)}m ${c.status}`).join(" | "));

for (const a of cards.slice(0, 6)) {
  const src = (a.phase1?.sources || a.phase2?.sources || []) as string[];
  const ids = afTeamIdsFromSources(src);
  console.log("\n==", a.input.home, a.input.away, a.status, "ids", ids);
  ...
  console.log("xiReady", res.overlay.xiReady, "err", res.error, "note", (res.overlay.note || "").slice(0, 180));
  console.log("inj", (res.overlay.injuries || "").slice(0, 220));
}
